...
public class NNLSSolver
{
	...
	{
		System.out.println("Inside solveNNLS" );
		 ...
		 */
		//for(int iterations = 0; iterations < 0.01*A.getColumnDimension()*A.getRowDimension(); iterations++)
		for(int iterations = 0; iterations <7000; iterations++)
		{
			System.out.println(iterations+":"+z.size() + "::" + p.size());
			...
			while(!allPositive)
			{
				...
				//if(Ep.rank()<Ep.getColumnDimension() & Ep.rank()<Ep.getRowDimension()) break;/////////////////////////////////////////////////////////////////////////////////4th July
				...
				else
				{
					...
					for (i = p.size() - 1; i >= 0; i--)
						if (Math.abs(x.get(p.get(i),0)) < 1e-5)//Close enough to zero, no?
							z.add(p.remove(i));
				}
			}
		}
